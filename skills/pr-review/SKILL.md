...
| `deploy-on-aws:awsiac` | When reviewing CloudFormation, CDK constructs, or CDK-generated templates — validate templates (`validate_cloudformation_template`), check compliance (`check_cloudformation_template_compliance`), look up construct props and CDK best practices (`cdk_best_practices`, `search_cdk_documentation`, `search_cdk_samples_and_constructs`) |
| `deploy-on-aws:awsknowledge` | When reviewing AWS service integration code — verify API behavior, limits, error codes via `read_documentation`, `search_documentation`, `recommend`, `get_regional_availability` |
